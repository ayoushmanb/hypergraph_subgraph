...
get.val2 <- function(n, m, n.prob, sub.rep, s.m, apx_itr,apx_itr_sub, d){
  ...
  # Subsamplimg itr for each hyper graph
  sub.triangle.ct <- lapply(1:sub.rep, function(sub_itr){
    ...
    sub.triangle <-  color_triangles_count_3(sub.hyp.set, s.m, n, apx_itr_sub, filter_id)
    return(sub.triangle)
  })
...
# type3.triangle.true = result from get.val1
# type3.triangle.sub = result from get.val2
# m = number of hyperedges
...
type3.triangle.sub.ci <- function(type3.triangle.true, type3.triangle.sub,
                                  m, apx_itr, apx_itr_sub, sub.MC.rep, 
                                  alpha = 0.05){
  return(mean(unlist(lapply(1:sub.MC.rep, function(ii){
    z <- qnorm(1-alpha/2)
    # True value
    trueval <- mean(unlist(type3.triangle.true)/apx_itr)
    
    sub_res_list <- unlist(type3.triangle.sub[[ii]])
